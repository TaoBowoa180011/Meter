...
def save_mask_inference_model(dirname,
                              ...
                              program_only=False):
    """
    Prune the given `main_program` to build a new program especially for Model,
    ...
    Note:
        The :code:`dirname` is used to specify the folder where Model model
    ...
    Args:
        dirname(str): The directory path to save the Model model.
        ...
        target_vars(list[Variable]): list of Variable. Variables from which we can get
                                     Model results.
        executor(Executor): The executor that saves the Model model. You can refer
                            to :ref:`api_guide_executor_en` for more details.
        main_program(Program, optional): The original program, which will be pruned to
                                         build the Model model. If is setted None,
                                         the global default :code:`_main_program_` will be used.
                                         Default: None.
        model_filename(str, optional): The name of file to save the Model program
        ...
        export_for_deployment(bool): If True, programs are modified to only support
                                     direct Model deployment. Otherwise,
                                     more information will be stored for flexible
                                     optimization and re-training. Currently, only
                                     True is supported.
                                     Default: True.
        program_only(bool, optional): If True, It will save Model program only, and do not
    ...
    Examples:
        .. code-block:: python
            ...
            # Save Model model. Note we don't save label and loss in this example
            fluid.io.save_inference_model(dirname=path,
                                          feeded_var_names=['img'],
                                          target_vars=[predict],
                                          executor=exe)

            # In this example, the save_inference_mode Model will prune the default
            # main program according to the network's input node (img) and output node(predict).
            # The pruned Model program is going to be saved in the "./infer_model/__model__"
    ...
    for op in all_ops:
        if op.type == 'auc':
            warnings.warn(
                "please ensure that you have set the auc states to zeros before saving Model model"
            )
            break
    ...
    # it can only be loaded for Model directly. If it's false, the whole
